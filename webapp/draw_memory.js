...
function draw_memory () {
  ...
  var div_explain = div_memory.append('div')
    ...
    .html('Click [draw memory].  That renders the memory layout of the first 2048 bytes of the NES above.  The values are the brightness of the rectangle, and a blue stroke indicates that memory address is active.  Click the memory address to add a probe and a doper.  The probe will update whenever the value is read.  For some addresses this is only a handful of times per level, others are read 100 times per frame.  Click activate to change the value to whatever you have set at the slider.  The value of the probe should be locked to the value set with the slider, indicating the memory is being replaced by your custom value.')
